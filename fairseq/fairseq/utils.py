...
def sum_norm(x, dim):
    sums = torch.sum(x, dim).unsqueeze(dim)
    temp = torch.div(x, sums)
    temp1 = torch.where(torch.isnan(temp), torch.ones_like(temp), temp)
    normed = torch.where(torch.isinf(temp1), torch.ones_like(temp1), temp1)
    return normed

def max_norm(x, dim):
    maxes = torch.max(x, dim).values.unsqueeze(dim)
    return torch.div(x, maxes)
 
def num_norm(x, dim):
    return torch.div(x, x.shape[dim])