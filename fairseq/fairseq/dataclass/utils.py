...
import os
import re
from argparse import ArgumentError, ArgumentParser, Namespace, ArgumentTypeError
...
def gen_parser_from_dataclass(
    ...
    def argparse_name(name: str):
        ...
        if name == "_name":
            # private member, skip
            return None
        # full_name = "--" + name.replace("_", "-") #TODO the original parser name
        full_name = "--" + name
    ...
    def get_kwargs_from_dc(
        ...
        if isinstance(field_default, str) and field_default.startswith("${"):
            ...
            if (
                ...
                # kwargs["dest"] = dataclass_instance._get_name(k)    # todo: add no- prefix argument
                # kwargs["action"] = (
    ...
    for k in dataclass_instance._get_all_attributes():
        ...
        # if field_type is bool:    # todo: add no- prefix argument
        #     field_args_f = ['--'+'no-'+field_name[2:]]
        #     alias = dataclass_instance._get_argparse_alias(k)
        #     if alias is not None:
        #         field_args_f.append('-no-'+alias[1:])
        ...
        try:
            ...
            # if field_type is bool:    # todo: add no- prefix argument
            #     kwargs['action']=("store_false" if kwargs['action'] is "store_true" else "store_true")
            #     parser.add_argument(*field_args_f, **kwargs)
