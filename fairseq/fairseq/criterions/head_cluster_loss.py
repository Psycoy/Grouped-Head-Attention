...
@register_criterion(
    "label_smoothed_cross_entropy_headclustering", dataclass=LabelSmoothedCrossEntropyCriterionConfig_headclustering
)
class LabelSmoothedCrossEntropyCriterion_headclustering(FairseqCriterion):
    def __init__(
        ...
        self.Supervise_mode = cfg.Supervise_mode
        
        self.catece = cfg.cate_coefficient
    ...
    def add_cateloss(self, net_output, Regu):
        Regu_init = float(Regu.data)
        
        # pseudo labels and cate_outs
        if self.cluster_matrix == "Q":
            raise NotImplementedError
        elif self.cluster_matrix == "K":
            raise NotImplementedError
        elif self.cluster_matrix == "V":
            head_labels_layers = torch.stack(net_output[1]["decoder_head_matrices"]["head_labels_V"])
            cateouts_layers = torch.stack(net_output[1]["decoder_head_matrices"]["v_cate_out"])
        elif self.cluster_matrix == "attn_matrix":
            raise NotImplementedError
        elif self.cluster_matrix == "headout":
            head_labels_layers = torch.stack(net_output[1]["decoder_head_matrices"]["head_labels_headout"])
            cateouts_layers = torch.stack(net_output[1]["decoder_head_matrices"]["headout_cate_out"])

        layers1, headnum1, clses1 = cateouts_layers.shape
        layers2, headnum2 = head_labels_layers.shape
        
        assert layers1==layers2 and headnum1==headnum2, f"layers1==layers2 and headnum1==headnum2: {layers1, layers2, headnum1, headnum2}"
        assert self.N_head_clusters==clses1, f"self.N_head_clusters==clses1: {self.N_head_clusters, clses1}"
        
        Regu += self.catece * self.CEloss(cateouts_layers.view(layers1 * headnum1, clses1), head_labels_layers.view(layers2 * headnum2, ))
        # print(f"cateouts_layers.view(layers1 * headnum1, clses1), head_labels_layers.view(layers2 * headnum2, ): {cateouts_layers.view(layers1 * headnum1, clses1), head_labels_layers.view(layers2 * headnum2, )}")
        # print(f"self.catece, CELOSS: {self.catece, self.CEloss(cateouts_layers.view(layers1 * headnum1, clses1), head_labels_layers.view(layers2 * headnum2, ))}")
        # print(f"Regu, Regu-Regu_init: {Regu, Regu-Regu_init}")
        return Regu, float(Regu.data)-Regu_init
    ...
    def forward(self, model, sample, reduce=True):
        ...
        if model.cfg.efficient_multihead_attention:
            if self.Supervise_mode == "classification":
                loss, Cluster_loss = self.add_cateloss(net_output, loss)
            elif self.Supervise_mode == "reconstruction":
