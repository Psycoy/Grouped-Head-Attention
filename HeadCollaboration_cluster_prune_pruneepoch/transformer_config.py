...
class EncDecBaseConfig(FairseqDataclass):
    ...
    layers: int = field(default=6, metadata={"help": "number of encoder layers"})
...
class DecoderConfig(EncDecBaseConfig):
    layers: int = field(default=6, metadata={"help": "number of decoder layers"})  #TODO 这行原本没有（如果子类中的field与基类同名，那么子类将会无条件覆盖基类）
...
class TransformerConfig(FairseqDataclass):
    efficient_multihead_attention: bool = field(
        default=False,
        metadata={"help": "Whether to use multihead attention variant for self-attention."},
    )
    
    attention_variant: ChoiceEnum(['HWA', 'head_cluster']) = field(
        default="HWA",
        metadata={"help": "Attention variant script to use"},
    )
    
    use_efficient_en_de_attn: bool = field(
        default=False,
        metadata={"help": "Whether to use the attention variant for the encoder-decoder attention."},
    )
    
    initialize_FFNs: bool = field(
        default=False,
        metadata={"help": "Whether to initialize the fully-connected layers in the transformer."},
    )
    
    use_dkscaling_HWA: bool = field(
        default=False,
        metadata={"help": "Whether to scale the qk dot product by dk^(-0.5) in HWA."},
    )
    
    residual_HWA: bool = field(
        default=False,
        metadata={"help": "Whether to use residual structure for HWA."},
    )
    
    residual_HWA_coefficient: float = field(default=1.0, metadata={"help": "The residual coefficient for HWA."})
    
    largeqkflatten: bool = field(
        default=False,
        metadata={"help": "Whether to use full information for attention map calculation."},
    )
    
    abandon_outputprojection: bool = field(
        default=False,
        metadata={"help": "Whether to abandon the output projection of original attention."},
    )
    
    abandon_HWAoutputprojection: bool = field(
        default=False,
        metadata={"help": "Whether to abandon the output projection of HWA."},
    )
    
    abandon_vprojection: bool = field(
        default=False,
        metadata={"help": "Whether to abandon the projection of v in HWA (using the output of MHA as v directly)."},
    )
    
    abandon_MHA_feedforward_encoder: bool = field(
        default=False,
        metadata={"help": "Whether to abandon the feedforward layers in Multihead attention layer of transformer encoder."},
    )
    
    abandon_MHA_feedforward_decoder: bool = field(
        default=False,
        metadata={"help": "Whether to abandon the feedforward layers in Multihead attention layer of transformer decoder."},
    )
    
    HWA_head_num: int = field(
        default=1,
        metadata={"help": "Number of HWA heads."},
    )
    
    use_HWA_norm: bool = field(
        default=False,
        metadata={"help": "Whether to use the layer norm at the output of HWA."},
    )
    
    epoch_start_to_prune: int = field(
        default=-1,
        metadata={"help": "Which epoch it starts to prune the other heads, only keeping the head closest to the cluster centre. (prune means making the output of target head zero)"},
    )
    
    need_prune: bool = field(
        default=False,
        metadata={"help": "Whether to prune the other heads, only keeping the head closest to the cluster centre. (prune means making the output of target head zero)"},
    )
    
    keep_updating_cluster: bool = field(
        default=False,
        metadata={"help": "Whether to keep updating the cluster after the prune starts. If not, the heads pruned can be seen as totally dropped."},
    )
    
    kmeans_distance_metric: ChoiceEnum(['euclidean', 'cosine']) = field(
        default='euclidean',
        metadata={"help": "Attention variant script to use"},
    )
    
    use_fake_kmeans: bool = field(
        default=False,
        metadata={"help": "Use a fake kmeans with no body, but return results with same forms. Just for testing."},
    )
    
    voting_to_prune: bool = field(
        default=False,
        metadata={"help": "Whether to design the prune based on multiple batches."},
    )
    
    collecting_indicators: bool = field(
        default=False,
        metadata={"help": "Whether it is collecting indicators."},
    )
