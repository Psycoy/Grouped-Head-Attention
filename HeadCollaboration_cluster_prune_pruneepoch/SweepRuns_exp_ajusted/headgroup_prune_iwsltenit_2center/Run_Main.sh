...
DataPath="iwslt14.tokenized.en-it"  # TODO: Change dataset path accordingly
CUDA_VISIBLE_DEVICES=2             # TODO: Change dataset path accordingly
...
do
    IFS='=' read -r -a ARGARRAY <<< $ARG
    if [ ${ARGARRAY[0]} = "--_patience" ]
    then
        _patience=${ARGARRAY[1]}
        echo "_patience: ${_patience}"
        break
    fi
done
...
# do
#     IFS='=' read -r -a ARGARRAY <<< $ARG
#     if [ ${ARGARRAY[0]} = "--_attention_variant" ]
#     then
#         _attention_variant=${ARGARRAY[1]}
#         echo "_attention_variant: ${_attention_variant}"
#         break
#     fi
# done
