    ...
    def forward(
        ...
        if self.cfg.efficient_multihead_attention:
            ...
            if self.cfg.debug_mode:
                sizedict_transformerbase1={}
                sizedict_transformerbase2={}
                sizedict_transformerbase3={}
                for key in encoder_head_matrices.keys():
                    sizedict_transformerbase1[key]=[encoder_head_matrices[key][i].size() for i in range(len(encoder_head_matrices[key]))]
                    # sizedict_transformerbase1_len = len(sizedict_transformerbase1[key])
                for key in decoder_out[1]["decoder_head_matrices"].keys():
                    sizedict_transformerbase2[key]=[decoder_out[1]["decoder_head_matrices"][key][i].size() for i in range(len(decoder_out[1]["decoder_head_matrices"][key]))]
                    # sizedict_transformerbase2_len = len(sizedict_transformerbase2[key])
                for key in decoder_head_matrices.keys():
                    sizedict_transformerbase3[key]=[decoder_head_matrices[key][i].size() for i in range(len(decoder_head_matrices[key]))]
                    # sizedict_transformerbase3_len = len(sizedict_transformerbase3[key])
