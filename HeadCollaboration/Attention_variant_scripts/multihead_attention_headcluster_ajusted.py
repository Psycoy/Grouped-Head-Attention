...
from kmeans_pytorch import kmeans, kmeans_fake
...
class MultiheadAttention_efficient(nn.Module):
    ...
    def forward(
        ...
        if self.cfg.cluster_matrix == 'Q':
            ...
            if self.cfg.use_fake_kmeans:
                print("Using fake kmeans!")
                labels_Q, cluster_centers_Q = kmeans_fake(q_heads, num_clusters=self.cfg.N_head_clusters, seed=0, device=torch.device(q_heads.device))
            else:
                labels_Q, cluster_centers_Q = kmeans(q_heads, num_clusters=self.cfg.N_head_clusters, seed=0, device=torch.device(q_heads.device))
        ...
        if k is not None:
            ...
            if self.cfg.cluster_matrix == 'K':
                ...
                if self.cfg.use_fake_kmeans:
                    print("Using fake kmeans!")
                    labels_K, cluster_centers_K = kmeans_fake(k_heads, num_clusters=self.cfg.N_head_clusters, seed=0, device=torch.device(k_heads.device))
                else:
                    labels_K, cluster_centers_K = kmeans(k_heads, num_clusters=self.cfg.N_head_clusters, seed=0, device=torch.device(k_heads.device))
        ...
        if v is not None:
            ...
            if self.cfg.cluster_matrix == 'V':
                ...
                if self.cfg.use_fake_kmeans:
                    print("Using fake kmeans!")
                    labels_V, cluster_centers_V = kmeans_fake(v_heads, num_clusters=self.cfg.N_head_clusters, seed=0, device=torch.device(v_heads.device))
                else:
                    labels_V, cluster_centers_V = kmeans(v_heads, num_clusters=self.cfg.N_head_clusters, seed=0, device=torch.device(v_heads.device))
        ...
        if self.cfg.cluster_matrix == 'attn_matrix':
            ...
            if self.cfg.use_fake_kmeans:
                print("Using fake kmeans!")
                labels_attention_matrices, cluster_centers_attention_matrices = kmeans_fake(attention_matrices_heads, num_clusters=self.cfg.N_head_clusters, seed=0, device=torch.device(attention_matrices_heads.device))
            else:
        ...
        if self.cfg.cluster_matrix == 'headout':
            ...
            if self.cfg.use_fake_kmeans:
                print("Using fake kmeans!")
                labels_headout, cluster_centers_headout = kmeans_fake(headout_heads, num_clusters=self.cfg.N_head_clusters, seed=0, device=torch.device(headout_heads.device))
            else:
                labels_headout, cluster_centers_headout = kmeans(headout_heads, num_clusters=self.cfg.N_head_clusters, seed=0, device=torch.device(headout_heads.device))    
...
if __name__ == '__main__':
    HWA = MultiheadAttention_efficient(None, 512, 8, self_attention = True)
    query = torch.rand(9, 64, 512)
    output, _ = HWA(query, query, query)
